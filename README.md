...
- **命令卡片** — `/status`, `/help`, `/commands` 等原生命令也显示为交互卡片（实验性）
...
插件现已支持为原生命令（`/status`, `/help`, `/commands`）显示交互卡片：

- **白名单限制**：仅支持上述 3 个命令，其他命令仍显示为文本
- **仅限飞书**：命令卡片仅在飞书/Lark 平台显示，其他平台显示文本
- **可能重复**：由于架构限制，短暂时间内可能同时收到卡片和文本消息
- **降级处理**：卡片发送失败时自动降级为文本响应

技术细节和架构分析见 [COMMAND_CARDS_ANALYSIS.md](COMMAND_CARDS_ANALYSIS.md)。
