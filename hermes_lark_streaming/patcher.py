...
def _start_hook(indent: str) -> str:
    return _make_hook(
        ...
        [
            ...
            "        _lark_anchor_id = getattr(event, 'reply_to_message_id', None)",
            "        if not _lark_anchor_id and getattr(source, 'thread_id', None):",
...
def _complete_hook(indent: str) -> str:
    return _make_hook(
        ...
        [
            ...
            "        if on_message_needs_text_fallback(message_id=_lark_completion_id):",
...
def _interrupt_hook(indent: str) -> str:
    return _make_hook(
        ...
        [
            ...
            "        _lark_next_anchor_id = getattr(pending_event, 'reply_to_message_id', None)",
            "        _lark_next_source = getattr(pending_event, 'source', None) or next_source",
            "        if (not _lark_next_anchor_id and pending_event is not None",
            "                and getattr(_lark_next_source, 'thread_id', None)):",
            "            _lark_next_anchor_id = self._reply_anchor_for_event(pending_event)",
